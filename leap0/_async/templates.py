...
class AsyncTemplatesClient:
    ...
    async def create(self, *, name: str, uri: str, credentials: RegistryCredentials | RegistryCredentialsDict | None = None) -> Template:
        ...
        data = cast(UploadTemplateResponseDict, await self._transport.request_json("POST", "/v1/template", json=payload, expected_status=201))
        return Template.from_dict(data)

    @intercept_errors("Failed to rename template: ")
    async def rename(self, template_id: str, *, name: str, http_timeout: float | None = None) -> None:
        ...
        await self._transport.request("PATCH", f"/v1/template/{template_id}", json=payload, expected_status=204, timeout=http_timeout)

    @intercept_errors("Failed to delete template: ")
    async def delete(self, template_id: str, http_timeout: float | None = None) -> None:
        ...
        Args:
            ...
            http_timeout: Optional HTTP request timeout in seconds for this SDK call.
        """
        await self._transport.request("DELETE", f"/v1/template/{template_id}", expected_status=204, timeout=http_timeout)
