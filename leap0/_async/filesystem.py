...
class AsyncFilesystemClient:
    ...
    async def mkdir(self, sandbox: SandboxRef, *, path: str, recursive: bool = False, permissions: str | None = None, http_timeout: float | None = None) -> None:
    ...
    async def write_bytes(self, sandbox: SandboxRef, *, path: str, content: bytes, permissions: str | None = None, http_timeout: float | None = None) -> None:
    ...
    @intercept_errors("Failed to write file: ")
    async def write_file(self, sandbox: SandboxRef, *, path: str, content: str, encoding: str = "utf-8", permissions: str | None = None, http_timeout: float | None = None) -> None:
        ...
        Example:
            ...
            ```
        """
        await self.write_bytes(sandbox, path=path, content=content.encode(encoding), permissions=permissions)

    @intercept_errors("Failed to write files: ")
    async def write_files_bytes(self, sandbox: SandboxRef, *, files: dict[str, bytes], http_timeout: float | None = None) -> None:
    ...
    @intercept_errors("Failed to write files: ")
    async def write_files(self, sandbox: SandboxRef, *, files: dict[str, str], encoding: str = "utf-8", http_timeout: float | None = None) -> None:
        ...
        Args:
            ...
            http_timeout: Optional HTTP request timeout in seconds for this SDK call.
        """
        await self.write_files_bytes(sandbox, files={p: c.encode(encoding) for p, c in files.items()})
    ...
    async def read_files_bytes(self, sandbox: SandboxRef, *, paths: list[str]) -> dict[str, bytes]:
        ...
        Returns:
            object: Result returned by this operation.
        """
        response = await self._transport.request("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/read-files", json={"paths": paths})
    ...
    async def read_files(self, sandbox: SandboxRef, *, paths: list[str], encoding: str = "utf-8", http_timeout: float | None = None) -> dict[str, str]:
        ...
        Returns:
            object: Result returned by this operation.
        """
        return {path: content.decode(encoding) for path, content in (await self.read_files_bytes(sandbox, paths=paths)).items()}

    @intercept_errors("Failed to delete: ")
    async def delete(self, sandbox: SandboxRef, *, path: str, recursive: bool = False, http_timeout: float | None = None) -> None:
    ...
    async def glob(self, sandbox: SandboxRef, *, path: str, pattern: str, exclude: list[str] | None = None, http_timeout: float | None = None) -> list[str]:
        ...
        if exclude is not None:
            payload["exclude"] = exclude
        data = cast(GlobResponseDict, await self._transport.request_json("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/glob", json=payload, timeout=http_timeout))
        return list(data.get("items", []))

    @intercept_errors("Failed to grep: ")
    async def grep(self, sandbox: SandboxRef, *, path: str, pattern: str, include: str | None = None, exclude: list[str] | None = None, http_timeout: float | None = None) -> list[SearchMatch]:
        ...
        if exclude is not None:
            payload["exclude"] = exclude
        data = cast(GrepResponseDict, await self._transport.request_json("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/grep", json=payload, timeout=http_timeout))
        return [SearchMatch.from_dict(item) for item in data.get("items", [])]

    @intercept_errors("Failed to edit file: ")
    async def edit_file(self, sandbox: SandboxRef, *, path: str, edits: list[FileEdit], http_timeout: float | None = None) -> EditFileResult:
        ...
        Args:
            ...
            http_timeout: Optional HTTP request timeout in seconds for this SDK call.
        """
        data = cast(EditFileResponseDict, await self._transport.request_json(
            "POST",
    ...
    async def edit_files(self, sandbox: SandboxRef, *, paths: list[str], find: str, replace: str = "", http_timeout: float | None = None) -> list[EditResult]:
        ...
        """
        data = cast(EditFilesResponseDict, await self._transport.request_json(
            "POST",
    ...
    async def copy(
        ...
        *,
        ...
        await self._transport.request("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/copy", json=payload, expected_status=204, timeout=http_timeout)

    @intercept_errors("Failed to check path: ")
    async def exists(self, sandbox: SandboxRef, *, path: str, http_timeout: float | None = None) -> bool:
        ...
        """
        data = cast(ExistsResponseDict, await self._transport.request_json("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/exists", json={"path": path}, timeout=http_timeout))
        return bool(data.get("exists", False))

    @intercept_errors("Failed to get directory tree: ")
    async def tree(self, sandbox: SandboxRef, *, path: str, max_depth: int | None = None, exclude: list[str] | None = None, http_timeout: float | None = None) -> TreeResult:
        ...
        if exclude is not None:
            payload["exclude"] = exclude
        data = cast(TreeResponseDict, await self._transport.request_json("POST", f"/v1/sandbox/{sandbox_id_of(sandbox)}/filesystem/tree", json=payload, timeout=http_timeout))
        return TreeResult.from_dict(data)


async def _parse_multipart_response(content_type: str, body: bytes) -> dict[str, bytes]:
